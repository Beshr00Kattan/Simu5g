...
{
    // Real 3GPP Ethernet PDU Session work: ROHC header compression only
    // makes sense for an IP payload; an Ethernet PDU session packet has no
    // IP header to compress. Guarded on the real PacketProtocolTag (already
    // set upstream by GtpUser/IP2Nic/TrafficFlowFilter -- see their doc
    // entries), replacing the reference shim's ad hoc
    // "findTag<TsnEtherTag>() != nullptr" check with a check against
    // Simu5G/INET's own already-existing protocol-tag mechanism. See
    // doc/ethernet-pdu-session/11-pdcp-protocol-guard.md.
    // Uses findTag() (never throws), not getTag() (throws if absent): this
    // runs on the TX side where the tag is always present, but defensively
    // treating "absent" as "not Ethernet" costs nothing and avoids a crash
...
{
    // Real 3GPP Ethernet PDU Session work: mirror of the headerCompress()
    // guard above -- an Ethernet PDU session packet was never ROHC-compressed,
    // so it must never be run through decompression either (which would
    // otherwise misparse the real Ethernet frame bytes as a compressed IP
    // header). See doc/ethernet-pdu-session/11-pdcp-protocol-guard.md.
    // This runs on the RX side, after the packet has round-tripped through
    // RLC segmentation/reassembly (RlcUm creates new Packet objects for PDU
    // fragments, e.g. UmTxEntity::rlcPduMake()) -- findTag() (never throws)
    // is used rather than getTag() specifically because whether the original
    // PacketProtocolTag survives that round-trip was not verified by test
    // execution; treating an absent tag as "not Ethernet" (the historical
    // default) is the safe fallback either way.
    auto protocolTag = pkt->findTag<PacketProtocolTag>();
    if (protocolTag != nullptr && protocolTag->getProtocol() == &Protocol::ethernetMac)
        return;
...
void LtePdcpRrcBase::toDataPort(cPacket *pktAux)
{
    ...
    // Real 3GPP Ethernet PDU Session work: only force-tag ipv4 when the
    // packet actually is IPv4 -- an Ethernet PDU session packet already
    // carries its own real Protocol::ethernetMac tag (set upstream) and must
    // keep it, so IP2Nic::toIpUe() can tell the two cases apart. See
    // doc/ethernet-pdu-session/11-pdcp-protocol-guard.md. findTag() (not
    // getTag()) because this runs on the RX side, post-RLC round-trip -- see
    // the identical note in headerDecompress() above.
    auto protocolTag = pkt->findTag<PacketProtocolTag>();
    if (protocolTag == nullptr || protocolTag->getProtocol() != &Protocol::ethernetMac)
