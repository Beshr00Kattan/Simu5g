...
void NRPdcpRrcEnb::fromDataPort(cPacket *pktAux)
{
    ...
    // no IP address to resolve srcId/destId from -- destId was already
    // resolved once, from the real destination MAC, at IP2Nic::toStackBsEthernet()
    // and is carried here via FlowControlInfo.destId. D2D is not implemented
    // for this session type in this scope. See
    // doc/ethernet-pdu-session/10-ip2nic-pdcp-ethernet.md.
    ...
    // Real 3GPP Ethernet PDU Session work (multi-UE): every Ethernet PDU
    // session flow carries srcAddr=dstAddr=0 (there is no IP address), so keying
    // the connection table on addresses alone would return the SAME LCID for
    // every UE. The gNB's downlink RLC names its per-flow TX entity by LCID
    // ("UmTxEntity Lcid: N"), so a second UE reusing LCID 1 crashes with
    // "a submodule of the same name already exists". Use the already-resolved
    // destination UE id (FlowControlInfo.destId, set at IP2Nic::toStackBsEthernet)
    // as the connection key for the Ethernet case, so each UE gets a distinct
    // LCID and therefore a distinct RLC entity. The IP path is unchanged.
    unsigned int lcidKeyDst = isEthernet ? (unsigned int)destId : lteInfo->getDstAddr();

    LogicalCid mylcid;
    if ((mylcid = ht_->find_entry(lteInfo->getSrcAddr(), lcidKeyDst, lteInfo->getTypeOfService(), lteInfo->getDirection())) == 0xFFFF)
    {
        // LCID not found
        ...
        ht_->create_entry(lteInfo->getSrcAddr(), lcidKeyDst, lteInfo->getTypeOfService(), lteInfo->getDirection(), mylcid);
    }
...
{
    // Real 3GPP Ethernet PDU Session work: an Ethernet PDU session packet has
    // no IP address (dstAddr stays 0). Its destination UE MacNodeId was already
    // resolved from the real destination MAC upstream (IP2Nic::toStackBsEthernet()
    // / TrafficFlowFilter) and is carried in FlowControlInfo.destId. Return it
    // directly: the IP-based Binder lookup below would resolve dstAddr=0 to a
    // non-UE id and make NRTxPdcpEntity::deliverPdcpPdu throw
    // "the destination is not a UE...". This branch is reached because
    // NRTxPdcpEntity::setIds() calls getDestId() and would otherwise overwrite
    // the correct destId. See doc/ethernet-pdu-session/25-pdcp-ethernet-destid.md.
