...
{
    // Real 3GPP Ethernet PDU Session work: an Ethernet PDU session packet has
    // no Ipv4Header to peek below. It is delivered to this UE's l2App[] slot
    // (see Ue.ned) through the same "nl" dispatcher ipv4 packets already use
    // -- just tagged for Protocol::ethernetMac instead of ipv4 -- by reusing
    // prepareForIpv4()'s existing, already-protocol-parameterized tagging
    // logic rather than adding a new dispatch mechanism. See
    // doc/ethernet-pdu-session/10-ip2nic-pdcp-ethernet.md. findTag() (not
    // getTag()) because by this point the packet already carries a real
    // PacketProtocolTag from LtePdcpRrcBase::toDataPort() regardless of
    // session type -- but using findTag() here too costs nothing and keeps
    // this defensive against the same RLC-round-trip uncertainty noted there.
    // Real 3GPP Ethernet PDU Session work: detect an Ethernet PDU session frame
    // by its ACTUAL content -- a real inet::EthernetMacAddressFields chunk at
    // the front -- not by the PacketProtocolTag. The tag does NOT reliably
    // survive the UE-side RLC fragment/reassemble round-trip (confirmed by
    // live run: the frame arrives here with its EthernetMacAddressFields intact
    // but the tag already reset to ipv4 by LtePdcpRrcBase::toDataPort()), which
    // made the tag-only check fall through to peekAtFront<Ipv4Header>() and
    // throw "Cannot convert chunk from EthernetMacAddressFields to Ipv4Header".
    // hasAtFront<> is non-throwing, so this is safe to test first. The tag is
    // still honored when present (fast path). See
    // doc/ethernet-pdu-session/26-ip2nic-ue-content-detection.md.
    ...
    if (taggedEthernet || pkt->hasAtFront<EthernetMacAddressFields>())
    {
        ...
        // see doc/ethernet-pdu-session/27-mac-visibility-logging.md.
        const auto& addrFields = pkt->peekAtFront<EthernetMacAddressFields>();
        EV_ERROR << "[ETH-PDU][UE] ip2nic -- srcMac=" << addrFields->getSrc().str()
           << " destMac=" << addrFields->getDest().str()
           << " -> deliver to l2App (taggedEthernet=" << taggedEthernet << ")" << endl;
...
void IP2Nic::fromIpBs(Packet * pkt)
{
    ...
    // Real 3GPP Ethernet PDU Session work: an Ethernet-tagged packet has no
    // Ipv4Header to peek below, and handover forwarding/holding (which is
    // entirely IP-address-keyed) is not implemented for this session type in
    // this scope -- see doc/ethernet-pdu-session/10-ip2nic-pdcp-ethernet.md.
    {
        auto protocolTag = pkt->findTag<PacketProtocolTag>();
        if (protocolTag != nullptr && protocolTag->getProtocol() == &Protocol::ethernetMac) {
            toStackBsEthernet(pkt);
            return;
        }
    }
...
void IP2Nic::toStackBsEthernet(Packet* pkt)
{
    // Real 3GPP Ethernet PDU Session work: reads the real destination MAC
    // directly off the (still fully intact -- nothing has popped it) real
    // Ethernet frame, and resolves it to the destination UE's MacNodeId via
    // the same Binder-backed lookup TrafficFlowFilter's Ethernet branch uses
    // (see doc/ethernet-pdu-session/07-tff-ethernet-branch.md) -- there is no
    // IP address on this packet to derive it from instead.
    const auto& addrFields = pkt->peekAtFront<EthernetMacAddressFields>();
    const MacAddress& destMac = addrFields->getDest();
    const MacAddress& srcMac = addrFields->getSrc();
    MacNodeId destId = binder_->getNodeIdForMacAddress(destMac);
    if (destId == 0)
    {
        EV_ERROR << "[ETH-PDU][gNB] DROP -- srcMac=" << srcMac.str() << " destMac=" << destMac.str()
           << " (unknown/unregistered), dropping." << endl;
        delete pkt;
        return;
    }

    auto lteInfo = pkt->addTagIfAbsent<FlowControlInfo>();
    lteInfo->setSrcAddr(0);
    lteInfo->setDstAddr(0);
    lteInfo->setHeaderSize(0);
    // Real 3GPP Ethernet PDU Session work: destId is resolved here, once,
    // from the real MAC address -- NRPdcpRrcEnb::fromDataPort() reads this
    // pre-set value for Ethernet sessions instead of re-deriving it from an
    // (absent) IP address (see that file's doc entry).
    lteInfo->setDestId(destId);
    // This scope's Ethernet PDU session topology is single-cell, pure-NR
    // (no dual connectivity / LTE fallback), so the existing IP-address-
    // driven markPacket() bearer-selection logic (SS5.6.10.2's ToS-threshold
    // policy) does not apply and is bypassed; useNR is forced directly.
    lteInfo->setUseNR(true);
    // typeOfService carries the frame's real VLAN PCP when present (real
    // 802.1Q priority, peeked non-destructively so the tag still reaches the
    // UE intact), falling back to 0. Not yet wired into the NR MAC
    // scheduler's own QoS prioritization -- see the final report's scope
    // section for why real PCP-based scheduling is documented future work.
    const auto& vlanHeader = pkt->peekAt<Ieee8021qTagEpdHeader>(addrFields->getChunkLength());
    lteInfo->setTypeOfService(vlanHeader->getPcp());
    ...
    // gNB-side MAC-to-UE resolution that feeds the radio stack (PDCP/RLC/MAC).
    EV_ERROR << "[ETH-PDU][gNB] FORWARD -- srcMac=" << srcMac.str() << " destMac=" << destMac.str()
       << " -> UE destId=" << destId << " pcp=" << (int)vlanHeader->getPcp() << " -> radio stack" << endl;
