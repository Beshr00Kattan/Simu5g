...
class IP2Nic : public omnetpp::cSimpleModule
{
  ...
  protected:
    ...
    // Real 3GPP Ethernet PDU Session work: Ethernet-frame equivalent of
    // toStackBs() -- resolves destId from the real destination MAC (via
    // Binder) instead of an Ipv4Header, since Ethernet PDU session packets
    // carry no IP address. See doc/ethernet-pdu-session/10-ip2nic-pdcp-ethernet.md.
    virtual void toStackBsEthernet(inet::Packet* datagram);
