...
//    }
    ...
    else if (stage == INITSTAGE_NETWORK_LAYER)
    {
        ...
        // Real 3GPP Ethernet PDU Session work: register this UE's declared
        // PDU session type (Ue.ned's "pduSessionType" parameter, default
        // "IPv4") with the Binder, right next to the existing MacNodeId
        // registration this is modeled on. Uses the same getAncestorPar()
        // idiom already used two lines below for extHostAddress -- see
        // doc/ethernet-pdu-session/05-ltemacue-registration.md.
        ...
        binder_->setPduSessionType(nodeId_, pduSessionType);

        // Real 3GPP Ethernet PDU Session work: for an Ethernet PDU session,
        // also register this UE's declared MAC address (Ue.ned's "tsnMac"
        // parameter), so TrafficFlowFilter/GtpUser can resolve a destination
        // MAC to this UE without an IP address (there is none for this
        // session type). See doc/ethernet-pdu-session/07-tff-ethernet-branch.md.
        if (pduSessionType == PduSessionType::ETHERNET) {
            const char *tsnMacStr = getAncestorPar("tsnMac").stringValue();
            if (strlen(tsnMacStr) > 0)
