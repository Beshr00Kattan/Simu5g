...
// Real 3GPP Ethernet PDU Session work (see doc/ethernet-pdu-session/04-binder-pdu-session-type.md):
// a PDU session's Type (TS 23.501 Table 5.6.10.2-1: IPv4, IPv6, Ethernet,
// Unstructured) is, for the two types this project models, a per-UE
// attribute known at configuration time (see Ue.ned's "pduSessionType"
// parameter) rather than the result of dynamic NAS PDU Session
// Establishment signaling -- Simu5G has no NAS/SM layer to extend, and
// this project's scope is the forwarding-plane behaviour driven by the
// session type, not the establishment procedure itself. This enum is
// intentionally just {IPv4, Ethernet}: IPv6/Unstructured are out of scope.
enum class PduSessionType { IPV4, ETHERNET };
...
class Binder : public omnetpp::cSimpleModule
{
  private:
    ...
    // Real 3GPP Ethernet PDU Session work: per-UE declared PDU session type,
    // registered once at attach time (see LteMacUe::initialize(), which
    // already registers macNodeId<->IP the same way). Queried by
    // TrafficFlowFilter to decide whether to classify a UE's traffic by
    // IP 5-tuple (existing behaviour, unchanged) or by Ethernet
    // destination MAC (new -- see TrafficFlowFilter's Ethernet branch).
    std::map<MacNodeId, PduSessionType> pduSessionType_;

    // Real 3GPP Ethernet PDU Session work: authoritative UE MAC address
    // registered at attach time (Ue.ned's "tsnMac" parameter), used by
    // TrafficFlowFilter to seed its (otherwise purely uplink-learned) real
    // inet::MacForwardingTable -- see doc/ethernet-pdu-session/07-tff-ethernet-branch.md
    // for why a provisioned fallback is needed given this project's downlink-only scope.
    std::map<inet::MacAddress, MacNodeId> ueMacAddressToNodeId_;
...
//    std::map<MacNodeId, L3Address> x2Address_;
  ...
  public:
    ...
    /**
     * Registers the declared PDU session type for a UE. Called once, at
     * attach time, from LteMacUe::initialize() -- the same place and the
     * same "configure once, register at attach" pattern already used for
     * setMacNodeId() above. See doc/ethernet-pdu-session/04-binder-pdu-session-type.md.
     */
    void setPduSessionType(MacNodeId nodeId, PduSessionType type)
    {
        pduSessionType_[nodeId] = type;
    }
    /**
     * Returns the declared PDU session type for a UE. Defaults to IPV4 for
     * any UE that never registered a type, so every pre-existing scenario
     * (which knows nothing about PduSessionType) is classified exactly as
     * it always was.
     */
    PduSessionType getPduSessionType(MacNodeId nodeId)
    {
        auto it = pduSessionType_.find(nodeId);
        return (it == pduSessionType_.end()) ? PduSessionType::IPV4 : it->second;
    }
    /**
     * Registers a UE's Ethernet PDU session MAC address, called from
     * LteMacUe::initialize() next to setPduSessionType(). See
     * doc/ethernet-pdu-session/07-tff-ethernet-branch.md.
     */
    void setUeMacAddress(inet::MacAddress mac, MacNodeId nodeId)
    {
        ueMacAddressToNodeId_[mac] = nodeId;
    }
    /**
     * Resolves a UE's MacNodeId from its registered Ethernet PDU session MAC
     * address. Returns 0 (matching getMacNodeId()'s "not found" convention)
     * if no UE registered that MAC.
     */
    MacNodeId getNodeIdForMacAddress(inet::MacAddress mac)
    {
        auto it = ueMacAddressToNodeId_.find(mac);
        return (it == ueMacAddressToNodeId_.end()) ? 0 : it->second;
    }
