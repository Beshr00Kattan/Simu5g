...
simsignal_t CbrSender::packetDelaySentSignal = registerSignal("packetDelaySentSignal");
simsignal_t CbrSender::appPacketSentSignal = registerSignal("appPacketSent");
...
void CbrSender::sendCbrPacket()
{
    ...
    // Network-wide packet delay/loss observer support: keyed by the CBR
    // packet's own real, serialized IDframe field (set above, just like
    // nframes_/payloadTimestamp), not a tag -- this is real payload data
    // that survives any lower-layer round-trip, unlike a Packet-level tag.
    // See doc/ethernet-pdu-session/28-packet-observers.md.
    emit(packetDelaySentSignal, (long)cbr->getIDframe());
    emit(appPacketSentSignal, 1L);
