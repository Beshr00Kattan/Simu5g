...
simsignal_t CbrReceiver::packetDelayReceivedSignal = registerSignal("packetDelayReceivedSignal");
simsignal_t CbrReceiver::appPacketReceivedSignal = registerSignal("appPacketReceived");
...
void CbrReceiver::handleMessage(cMessage *msg)
{
    ...
    // Network-wide packet delay/loss observer support: keyed by the same
    // real IDframe field CbrSender emitted on send -- see
    // doc/ethernet-pdu-session/28-packet-observers.md.
    emit(packetDelayReceivedSignal, (long)cbrHeader->getIDframe());
    emit(appPacketReceivedSignal, 1L);
