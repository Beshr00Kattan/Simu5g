...
class TrafficFlowFilter : public omnetpp::cSimpleModule
{
    ...
    // Real 3GPP Ethernet PDU Session work: real, reused INET learning/aging
    // MAC forwarding table (see Upf.ned). nullptr if this TrafficFlowFilter
    // instance was not given a "macForwardingTableModule" path (e.g. ENB/GNB
    // owner types, which never see Ethernet PDU session traffic in this scope).
    inet::MacForwardingTable* macForwardingTable_ = nullptr;
  ...
  protected:
    ...
    // Real 3GPP Ethernet PDU Session work -- see
    // doc/ethernet-pdu-session/07-tff-ethernet-branch.md and
    // doc/ethernet-pdu-session/15-real-switch-topology.md.
    // Classifies a real Ethernet frame (arrived via the real tsnSwitch/eth[0]
    // path, no IP/UDP involved) by destination MAC via the real
    // MacForwardingTable (learn on uplink, look up + flood-if-unknown on
    // downlink), and forwards it to GtpUser exactly like the IP branch does.
    void handleEthernetPacket(inet::Packet *pkt);
